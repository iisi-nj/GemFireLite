...
public class LocatorBean66 implements InitializingBean
{
  ...
  {
    startLocator(port, throwOnBindFailure);
  }
  ...
  public void startLocator(int port, boolean throwOnFail)
  {
    try
    {
//      locator = InternalLocator.startLocator(port, logFile, state, null, null, bind, prop, true, serverLocator, null);
//      locator= InternalLocator.startLocator(port,logFile, state, null, null, bind, true, prop, true, true, null);
      locator= InternalLocator.startLocatorAndDS(port, logFile, bind, prop);
      log.info("Locator starting at " + bind + ":" + port);
    }
    catch (Exception e)
    {
      log.info("Locator start failure for port[" + port + "]:", e);
    }
  }
