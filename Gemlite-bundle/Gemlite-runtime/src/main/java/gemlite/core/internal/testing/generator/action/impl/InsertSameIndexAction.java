...
public class InsertSameIndexAction implements OutputAction
{
	@Override
	public Object doAct(OutputTemplate template, Object argument)
	{
		...
		Map<String, String> usedIndexKey = pool.getUsedIdxKey();
		Set<String> usedIdxKeySet = usedIndexKey.keySet();
		...
		try
		{
			...
			for(int i=0; i<arr.length; i++)
			{
				...
				else
				{
					...
					String rValue;
					if(usedIdxKeySet.contains(fName))
					{
						rValue = usedIndexKey.get(fName);
					}
					else
					{
